...
export const meta = {
  ...
  date: "2019-08-01",
  description: "The data-masking crusader",
  published: true,
};
...
- Nothing is saved, until you press _save_.
...
Since the schema says we have to pass an ID of `ID!` when using `node(id: ID!)`, we _need_ to pass a value to `id` when using it in our query.
...
function Person({ initialPerson }) {
  ...
  return <PersonSelect value={person} onChange={setPerson} />;
}
...
This means, that if we want access to any properties on `Person`, we have to place them directly in the `DialogContainerPerson` _and_ in the `DialogContainerTask` person fragment.
...
The answer is **_Isolation, and reusability_**. Whenever I select a `Person` in the context of my dialog, I will need to _know_ which fields are required by the `Person` component and it's respective sub-fields.
...
Our _only_ option is to get rid of the `Notes` fragment entirely, and move _all_ the fields from notes, into the `DialogContainerTask` fragment.

_Sigh_ Fine, Relay - whatever you say.
...
We'll just have to write everything twice, and since we're **unable** to use fragments in some cases, _well_...
...
But what if we could query a _local_ node with fragments? Certainly an idea worth exploring.  
  ...
  function Notes({ activity }) {
    function onCreate() {
      commitLocalUpdate(environment, (store) => {
        ...
        temp.setLinkedRecords(
          [...(temp.getLinkedRecords("notes") || []), record],
          "notes"
        );
      });
    }
    ...
    function onDelete(note) {
      commitLocalUpdate(environment, (store) => {
        ...
        temp.setLinkedRecords(
          temp
            .getLinkedRecords("notes")
            .filter((x) => x.getDataID() !== note.id),
          "notes"
        );
      });
    }
...
I also want you to notice, that we no longer need to memoize the component, since no unnecessary re-renders are being triggered by the parent. Because LocalQueryRenderer is subscribed to any changes on `temp.notes`, updates are pushed **_directly within our local component scope._**
...
Furthermore, we were able to _remove_ all instances of `React.memo`, and achieve _local component state_, which is updated automatically, whenever changes occur to any of the fields specified in the component fragment.

That is a _big_ deal. It allows you to implicitly update local component state, from **outside the component**.

![Mind blown](https://media.giphy.com/media/12AZjJcRGhu9nG/giphy.gifhttps://media.giphy.com/media/12AZjJcRGhu9nG/giphy.gif)

No, **seriously**. The explicit, yet implicit beauty of this pattern _figuratively_ blows my mind.
