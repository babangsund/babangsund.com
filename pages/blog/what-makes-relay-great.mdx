...
export const meta = {
  ...
  date: "2019-07-18",
  description: "Is also it's greatest weakness.",
  published: true,
};
...
The gist of it, is that it allows you to subscribe to a stream of data (like from a cache, or a server), which can then be closed at a later time. If you've ever used [RxJS](https://github.com/ReactiveX/RxJS), this will be _very_ familiar to you.
...
_Don't delete data, unless we know it might be stale._
...
```jsx
function fetchQueryOuter(operation, variables, cacheConfig) {
...
```jsx {5,12,14,15}
function fetchQuery(operation, variables, cacheConfig, sink) {
  // Try to get data from cache on queries
  const fromCache = cache.get(queryID, variables);
  if (
    isQuery &&
...
How can we fix this?
One strategy, is to avoid fetching from server, unless we **know** that the data has _not_ been fetched after the most recent mutation.
