...
---
 ...
 great movie series, is a trilogy.  
...
With that being said, I want to share how I've recently used Relay to drastically simplify a complicated dialog architecture. Reading that, you might be thinking: "How can a dialog be complicated?"; and how could it possibly warrant the use of something like Relay state?
...
function DialogRenderer({ taskId }) {
	...
	<QueryRenderer
		    ...
		    query={graphql`query DialogQuery($taskId: ID!) {
			...
			render={({error,props}) => {
				...
				return (
					<DialogContainer task={props.task || null} />
				)
			}}
		/>
	)
...
The `initialPerson` comes from the fragment provided by the `DialogContainer`, which is used to set the initial state. We've leveraged the isolation of Relay's data-masking, as well as local component state to control any future changes. Great!
...
const PersonComponent = memo(({person, setTask}) => {
	return (
		<PersonSelect
			person={person}
			onChange={person => setTask(state => ({...state,person}))}
		 />
	)	
})
```
...
This means, that if we want access to the properties on `Person`, we will need to put them all in the `DialogContainer` fragment.

Let's pretend we need access to the `id` and `firstName` properties from `person`.
...
Let's introduce another problem, by adding another field to the `Task` type.  
A list of notes.
...
const Notes = memo(({notes, setTask}) => {
	...
	return (
		<div>
			...
			{notes.map(x => <span key={x.id}>x.body</span>)}
		</div>
	)	
})
...
And add it to the `DialogContainer`.
...
As you might imagine (which you'll have to, because I don't have all day), a query following this pattern, can get quite large with scale. I should point out, that any scalar fields on `Task`, will also need to be added to the soon-to-be mega query.
...
You know what would make this query infinitely more fun?  
*Nesting!*

Imagine we need access to the previous task in a series of follow-ups.
Call it `previousActivity`.
...
But what if we create a local entity, which we can fetch with fragments, in a **local query**?
...
function DialogContainer({ task }) {
  ...
  return <ActivityDialogView {...other} isLoading={!query} />;
	return (
		<div>
			<PersonComponent />
			<Notes />
...
let id = 0;
const Notes = createFragmentContainer(({ activity }) => {
